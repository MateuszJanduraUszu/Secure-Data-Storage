...
#pragma once
#ifndef _SDSDLL_CRYPTOGRAPHY_CIPHER_SYMMETRIC_AES256_CBC_HPP_
#define _SDSDLL_CRYPTOGRAPHY_CIPHER_SYMMETRIC_AES256_CBC_HPP_
...
_SDSDLL_BEGIN
// CLASS TEMPLATE _Aes256_cbc_fn
template <class _Simple_types>
class _Aes256_cbc_fn { // wrapper for the EVP AES-256 CBC functions
public:
    ...
    constexpr _Aes256_cbc_fn() noexcept     = default;
    _CONSTEXPR20 ~_Aes256_cbc_fn() noexcept = default;

    _Aes256_cbc_fn(const _Aes256_cbc_fn&) = delete;
    _Aes256_cbc_fn& operator=(const _Aes256_cbc_fn&) = delete;

    // tries to encrypt/decrypt the data
    _NODISCARD constexpr bool operator()(byte_type* const _Buf, size_type _Buf_size,
        const byte_type* const _Data, const size_type _Data_size, const key& _Key, const iv& _Iv,
        const cipher_task _Task, size_type* const _Plain_size = nullptr) const noexcept;
};

// CLASS TEMPLATE _Aes256_cbc_default_engine
template <class _Elem>
class _Aes256_cbc_default_engine { // default engine for the AES-256 CBC algorithm
private:
    static_assert(is_any_of_v<_Elem, char, uint8_t, wchar_t>, "Requires UTF-8/UTF-16 element type.");

    using _Simple_types = _Simple_cipher_types<_Elem, _Aes256_cbc_default_engine<_Elem>, aes_key<32>, iv<16>>;
    using _Fn           = _Aes256_cbc_fn<_Simple_types>;
...
public:
    ...
    constexpr _Aes256_cbc_default_engine(const _Elem* const _Data, const size_type _Size,
        const key& _Key, const iv& _Iv, const cipher_task _Task) noexcept;
    _CONSTEXPR20 ~_Aes256_cbc_default_engine() noexcept;

    _Aes256_cbc_default_engine() = delete;
    _Aes256_cbc_default_engine(const _Aes256_cbc_default_engine&) = delete;
    _Aes256_cbc_default_engine& operator=(const _Aes256_cbc_default_engine&) = delete;

    // returns encrypted/decrypted data or an empty string
    _NODISCARD constexpr byte_string bytes() const;

private:
    // calculates the buffer size for the AES-256 CBC encryption/decryption
...
template <class _Elem, class _Eng = _Aes256_cbc_default_engine<uint8_t>>
_SDSDLL_API _NODISCARD constexpr basic_string<_Elem> decrypt_aes256_cbc(
...
template <class _Elem, class _Eng = _Aes256_cbc_default_engine<_Elem>>
_SDSDLL_API _NODISCARD constexpr byte_string encrypt_aes256_cbc(
...
#endif // _SDSDLL_CRYPTOGRAPHY_CIPHER_SYMMETRIC_AES256_CBC_HPP_