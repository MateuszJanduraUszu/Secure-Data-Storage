...
_SDSDLL_BEGIN
// FUNCTION TEMPLATE _Argon2id_fn::operator()
template <class _Simple_types>
_NODISCARD constexpr bool _Argon2id_fn<_Simple_types>::operator()(byte_type* const _Buf,
...
// FUNCTION TEMPLATE _Argon2id_default_engine copy constructor/destructor
template <class _Elem>
constexpr _Argon2id_default_engine<_Elem>::_Argon2id_default_engine(const _Elem* const _Data,
...
template <class _Elem>
_CONSTEXPR20 _Argon2id_default_engine<_Elem>::~_Argon2id_default_engine() noexcept {}

// FUNCTION TEMPLATE _Argon2id_default_engine::bytes
template <class _Elem>
_NODISCARD constexpr typename _Argon2id_default_engine<_Elem>::byte_string
    _Argon2id_default_engine<_Elem>::bytes() const {
    return !_Mypair._Val1._Empty() ? byte_string{_Mypair._Val1._Get(), size} : byte_string{};
}

// auxilary macro for sdsdll::argon2id() function export/import
#define _EXPORT_OR_IMPORT_ARGON2ID(_Elem, _Str, ...)                        \
    template _SDSDLL_API _NODISCARD byte_string argon2id(_Str, __VA_ARGS__, \
        const salt<_Argon2id_default_engine<_Elem>>&, const size_t, const size_t, const size_t)

// FUNCTION TEMPLATE argon2id
template <class _Elem, class _Eng>
_NODISCARD constexpr byte_string argon2id(
...
_EXPORT_OR_IMPORT_ARGON2ID(char, const char* const, const size_t);
_EXPORT_OR_IMPORT_ARGON2ID(wchar_t, const wchar_t* const, const size_t);

template <class _Elem, class _Eng>
_NODISCARD constexpr byte_string argon2id(const _Elem* const _Data, const salt<_Eng>& _Salt,
    const size_t _Memory_amount, const size_t _Iterations, const size_t _Parallelism) {
...
_EXPORT_OR_IMPORT_ARGON2ID(char, const char* const);
_EXPORT_OR_IMPORT_ARGON2ID(wchar_t, const wchar_t* const);

template <class _Elem, class _Eng>
_NODISCARD constexpr byte_string argon2id(const basic_string_view<_Elem> _Data, const salt<_Eng>& _Salt,
    const size_t _Memory_amount, const size_t _Iterations, const size_t _Parallelism) {
    return _Hash_password<_Elem, _Eng>(
        _Data.data(), _Data.size(), _Salt, _Memory_amount, _Iterations, _Parallelism);
}

_EXPORT_OR_IMPORT_ARGON2ID(char, const string_view);
_EXPORT_OR_IMPORT_ARGON2ID(wchar_t, const wstring_view);

template <class _Elem, class _Eng>
_NODISCARD constexpr byte_string argon2id(const basic_string<_Elem>& _Data, const salt<_Eng>& _Salt,
    const size_t _Memory_amount, const size_t _Iterations, const size_t _Parallelism) {
    return _Hash_password<_Elem, _Eng>(
        _Data.c_str(), _Data.size(), _Salt, _Memory_amount, _Iterations, _Parallelism);
}

_EXPORT_OR_IMPORT_ARGON2ID(char, const string&);
_EXPORT_OR_IMPORT_ARGON2ID(wchar_t, const wstring&);
#undef _EXPORT_OR_IMPORT_ARGON2ID
_SDSDLL_END
