...
_SDSDLL_BEGIN
// FUNCTION TEMPLATE argon2id_traits::bytes_count
template <class _Elem>
_NODISCARD constexpr typename argon2id_traits<_Elem>::size_type
    argon2id_traits<_Elem>::bytes_count() noexcept {
    return bits / CHAR_BIT; // always 64 bytes
}

// FUNCTION TEMPLATE argon2id_traits::hash
template <class _Elem>
_NODISCARD constexpr bool argon2id_traits<_Elem>::hash(
    ...
    if _CONSTEXPR_IF (sizeof(_Elem) == 1) { // hash a UTF-8 password
        ...
        if (_Narrow.size() > 0xFFFF'FFFF) { // Argon2id can hash at most 2^32 - 1 bytes
            return false;
        }
...
template _SDSDLL_API struct argon2id_traits<char>;
template _SDSDLL_API struct argon2id_traits<unsigned char>;
template _SDSDLL_API struct argon2id_traits<wchar_t>;
_SDSDLL_END
