...
_SDSDLL_BEGIN
// FUNCTION TEMPLATE _Argon2d_fn::operator()
template <class _Simple_types>
_NODISCARD constexpr bool _Argon2d_fn<_Simple_types>::operator()(byte_type* const _Buf,
...
// FUNCTION TEMPLATE _Argon2d_default_engine copy constructor/destructor
template <class _Elem>
constexpr _Argon2d_default_engine<_Elem>::_Argon2d_default_engine(const _Elem* const _Data,
    ...
    if (!_Mypair._Val1._Empty()) { // buffer allocated, complete the task
        if _CONSTEXPR_IF (is_same_v<_Elem, char>) {
            ...
            _Release_on_failure(_Mypair._Val1, _Mypair._Get_val2()(
                _Mypair._Val1._Get(), size, _Narrow.c_str(), _Narrow.size(), _Salt,
                    salt_size, _Memory_amount, _Iterations, _Parallelism));
        }
    }
}

template <class _Elem>
_CONSTEXPR20 _Argon2d_default_engine<_Elem>::~_Argon2d_default_engine() noexcept {}

// FUNCTION TEMPLATE _Argon2d_default_engine::bytes
template <class _Elem>
_NODISCARD constexpr typename _Argon2d_default_engine<_Elem>::byte_string
    _Argon2d_default_engine<_Elem>::bytes() const {
    return !_Mypair._Val1._Empty() ? byte_string{_Mypair._Val1._Get(), size} : byte_string{};
}

// auxilary macro for sdsdll::argon2d() function export/import
#define _EXPORT_OR_IMPORT_ARGON2D(_Elem, _Str, ...)                        \
    template _SDSDLL_API _NODISCARD byte_string argon2d(_Str, __VA_ARGS__, \
        const salt<_Argon2d_default_engine<_Elem>>&, const size_t, const size_t, const size_t)

// FUNCTION TEMPLATE argon2d
template <class _Elem, class _Eng>
_NODISCARD constexpr byte_string argon2d(
...
_EXPORT_OR_IMPORT_ARGON2D(char, const char* const, const size_t);
_EXPORT_OR_IMPORT_ARGON2D(wchar_t, const wchar_t* const, const size_t);

template <class _Elem, class _Eng>
_NODISCARD constexpr byte_string argon2d(const _Elem* const _Data, const salt<_Eng>& _Salt,
    const size_t _Memory_amount, const size_t _Iterations, const size_t _Parallelism) {
...
_EXPORT_OR_IMPORT_ARGON2D(char, const char* const);
_EXPORT_OR_IMPORT_ARGON2D(wchar_t, const wchar_t* const);

template <class _Elem, class _Eng>
_NODISCARD constexpr byte_string argon2d(const basic_string_view<_Elem> _Data, const salt<_Eng>& _Salt,
    const size_t _Memory_amount, const size_t _Iterations, const size_t _Parallelism) {
    return _Hash_password<_Elem, _Eng>(
        _Data.data(), _Data.size(), _Salt, _Memory_amount, _Iterations, _Parallelism);
}

_EXPORT_OR_IMPORT_ARGON2D(char, const string_view);
_EXPORT_OR_IMPORT_ARGON2D(wchar_t, const wstring_view);

template <class _Elem, class _Eng>
_NODISCARD constexpr byte_string argon2d(const basic_string<_Elem>& _Data, const salt<_Eng>& _Salt,
    const size_t _Memory_amount, const size_t _Iterations, const size_t _Parallelism) {
    return _Hash_password<_Elem, _Eng>(
        _Data.c_str(), _Data.size(), _Salt, _Memory_amount, _Iterations, _Parallelism);
}

_EXPORT_OR_IMPORT_ARGON2D(char, const string&);
_EXPORT_OR_IMPORT_ARGON2D(wchar_t, const wstring&);
#undef _EXPORT_OR_IMPORT_ARGON2D
_SDSDLL_END
