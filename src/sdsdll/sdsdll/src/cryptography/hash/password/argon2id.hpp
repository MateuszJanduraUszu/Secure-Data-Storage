...
#pragma once
#ifndef _SDSDLL_CRYPTOGRAPHY_HASH_PASSWORD_ARGON2ID_HPP_
#define _SDSDLL_CRYPTOGRAPHY_HASH_PASSWORD_ARGON2ID_HPP_
...
_SDSDLL_BEGIN
// CLASS TEMPLATE _Argon2id_fn
template <class _Simple_types>
class _Argon2id_fn { // wrapper for the Botan::argon2() function
public:
    ...
    constexpr _Argon2id_fn() noexcept     = default;
    _CONSTEXPR20 ~_Argon2id_fn() noexcept = default;

    _Argon2id_fn(const _Argon2id_fn&) = delete;
    _Argon2id_fn& operator=(const _Argon2id_fn&) = delete;
...
// CLASS TEMPLATE _Argon2id_default_engine
template <class _Elem>
class _Argon2id_default_engine { // default engine for the Argon2id algorithm
private:
    static_assert(is_any_of_v<_Elem, char, wchar_t>, "Required UTF-8/UTF-16 element type.");

    using _Simple_types = _Simple_hash_types<_Elem, _Argon2id_default_engine<_Elem>>;
    using _Fn           = _Argon2id_fn<_Simple_types>;
...
public:
    ...
    constexpr _Argon2id_default_engine(const _Elem* const _Data, const size_type _Size,
    ...
    _CONSTEXPR20 ~_Argon2id_default_engine() noexcept;

    _Argon2id_default_engine() = delete;
    _Argon2id_default_engine(const _Argon2id_default_engine&) = delete;
    _Argon2id_default_engine& operator=(const _Argon2id_default_engine&) = delete;
...
template <class _Elem, class _Eng = _Argon2id_default_engine<_Elem>>
_SDSDLL_API _NODISCARD constexpr byte_string argon2id(
...
template <class _Elem, class _Eng = _Argon2id_default_engine<_Elem>>
_SDSDLL_API _NODISCARD constexpr byte_string argon2id(const _Elem* const _Data, const salt<_Eng>& _Salt,
    const size_t _Memory_amount = 16384, const size_t _Iterations = 8, const size_t _Parallelism = 1);
...
#endif // _SDSDLL_CRYPTOGRAPHY_HASH_PASSWORD_ARGON2ID_HPP_